  ...
  useEffect(() => {
    const handleResize = () => {
      if (chatWindowRef.current && isOpen) {
        ...
        if (isMobile) {
          chatWindowRef.current.style.left = '4px';
          chatWindowRef.current.style.right = '4px';
          chatWindowRef.current.style.width = 'calc(100vw - 8px)';
        }
      }
    };
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed left-2 bottom-2 sm:left-4 sm:bottom-4 md:left-6 md:bottom-6 z-50 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-emerald-500 to-lime-500 text-white flex items-center justify-center shadow-2xl hover:shadow-3xl transition-all duration-300 hover:scale-110 active:scale-95 group"
      ...
      <div
        ref={chatWindowRef}
        className={`fixed left-2 bottom-16 sm:left-4 sm:bottom-24 md:left-6 md:bottom-24 z-50 w-[calc(100vw-16px)] sm:w-80 md:w-96 bg-white rounded-xl sm:rounded-2xl shadow-2xl border border-gray-200 transition-all duration-300 ease-out ${
          isOpen
      ...
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-emerald-500 to-lime-500 rounded-t-xl sm:rounded-t-2xl p-3 sm:p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0">
                <h3 className="font-bold text-white text-sm sm:text-lg truncate">AgroChain AI</h3>
                <p className="text-emerald-100 text-[10px] sm:text-xs flex items-center gap-1 truncate">
                  <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-green-300 animate-pulse flex-shrink-0"></span>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {messages.length > 1 && (
                <button
                  onClick={clearChat}
                  className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-colors group/clear"
                  aria-label="Clear chat"
                  title="Clear chat"
                >
                  <Trash2 className="w-3 h-3 sm:w-4 sm:h-4 text-white group-hover/clear:scale-110 transition-transform" />
                </button>
              )}
        ...
        {/* Messages Container */}
        <div 
          className="h-64 sm:h-80 md:h-96 overflow-y-auto p-2 sm:p-4 space-y-2 sm:space-y-4 bg-gray-50/50"
          style={{
        ...
        >
          {messages.map((message) => (
            ...
            >
              {message.sender === 'bot' && (
                <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-emerald-400 to-lime-400 flex items-center justify-center flex-shrink-0">
              ...
              <div
                className={`max-w-[75%] sm:max-w-[70%] rounded-xl sm:rounded-2xl p-2 sm:p-3 ${
                  message.sender === 'user'
                    ? 'bg-gradient-to-r from-emerald-500 to-lime-500 rounded-br-none sm:rounded-br-none'
                    : 'bg-white border border-gray-200 rounded-bl-none sm:rounded-bl-none'
                }`}
              >
                <div 
          ...
          {isTyping && (
            <div className="flex gap-2 sm:gap-3 justify-start">
              <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-emerald-400 to-lime-400 flex items-center justify-center">
                <Bot className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
              </div>
              <div className="bg-white border border-gray-200 rounded-xl sm:rounded-2xl rounded-bl-none p-2 sm:p-3">
                <div className="flex items-center gap-1">
                  <Loader2 className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500 animate-spin" />
                  <span className="text-xs sm:text-sm text-gray-900">AgroChain AI is typing...</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="border-t border-gray-200 p-2 sm:p-4 bg-white">
          <div className="mb-2 sm:mb-3">
            <div className="flex items-center justify-between mb-1 sm:mb-2">
              <div className="flex items-center gap-1 sm:gap-2 text-xs text-gray-900">
                <HelpCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                <span className="font-medium text-xs sm:text-sm">Quick questions:</span>
              </div>
              ...
              {messages.length > 1 && (
                <button
                  onClick={clearChat}
                  className="sm:hidden flex items-center gap-1 text-xs text-gray-500 hover:text-emerald-600 transition-colors"
                  aria-label="Clear chat"
                >
              ...
              {quickQuestions.map((question) => {
                ...
                return (
                  <button
                    ...
                    className="text-left p-1.5 sm:p-2 rounded-lg border border-gray-200 hover:border-emerald-300 hover:bg-emerald-50 transition-all duration-200 text-[10px] xs:text-xs sm:text-xs text-gray-900 hover:text-emerald-700 flex items-center gap-1 sm:gap-2"
                  >
                    <Icon className="w-2.5 h-2.5 sm:w-3 sm:h-3 flex-shrink-0 text-gray-600" />
                    <span className="truncate text-left">{question.text}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex gap-1.5 sm:gap-2">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-3 sm:px-4 py-2 sm:py-3 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-xs sm:text-sm bg-gray-50 text-gray-900 placeholder-gray-500"
                disabled={isTyping}
              />
              {inputValue && (
                <button
                  onClick={() => setInputValue('')}
                  className="absolute right-10 sm:right-12 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600"
            ...
            <button
              ...
              className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-r from-emerald-500 to-lime-500 text-white flex items-center justify-center hover:shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
        ...
        <div className="border-t border-gray-200 p-2 sm:p-3 bg-gray-50/50 flex items-center justify-between">
          <p className="text-[10px] sm:text-xs text-gray-600 truncate">
            AI assistant • Tap to ask
          </p>
          
          {/* Mobile Help Hint */}
          <div className="sm:hidden flex items-center gap-1 text-[10px] text-emerald-600">
            <Smartphone className="w-2.5 h-2.5" />
            <span>Tap quick questions</span>
          </div>
          ...
          {messages.length > 1 && (
            <button
              onClick={clearChat}
              className="hidden sm:flex items-center gap-1 text-xs text-gray-500 hover:text-emerald-600 transition-colors"
              aria-label="Clear chat"
            >
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm sm:bg-black/10 sm:backdrop-blur-sm"
          onClick={handleClose}
        />
      )}
    </>
